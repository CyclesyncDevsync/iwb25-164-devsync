      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="mx-auto max-w-4xl w-full bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                  ...
                  <dd className="mt-1">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      material.status === MaterialStatus.DRAFT
                        ...
                        : material.status === MaterialStatus.PENDING_REVIEW || material.status === 'pending_verification'
                        ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400'
                        : material.status === MaterialStatus.APPROVED || material.status === 'approved' || material.status === 'confirmed'
              ...
              <div>
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                      {material.pricing.currency} {material.pricing.expectedPrice.toLocaleString()}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Minimum Price</dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                      {material.pricing.currency} {material.pricing.minimumPrice.toLocaleString()}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Negotiable</dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                      {material.pricing.negotiable ? 'Yes' : 'No'}
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              <div>
                ...
                {material.location.coordinates && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Coordinates: {material.location.coordinates.latitude}, {material.location.coordinates.longitude}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t dark:border-gray-600 rounded-b-lg">
            <button
              onClick={onClose}
              className="w-full sm:w-auto px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-600 transition-colors"
            >
              Close
            </button>
          </div>
