...
export default function MaterialManagement() {
  ...
  useEffect(() => {
    // Fetch materials from the backend API - optimized
    const fetchMaterials = async () => {
      try {
        ...
        const supplierId =
          ...
          "103ad4f1-8def-43e5-b72f-aa4c6d306bf4";
        ...
        if (response.ok) {
          ...
          const transformedMaterials = materialsArray.map(
            (submission: any) => ({
              ...
              condition: submission.condition
                ? (submission.condition.toLowerCase() as QualityGrade)
                : QualityGrade.GOOD,
              pricing: {
              ...
              status: (submission.submission_status ||
                "pending_review") as MaterialStatus,
              location: {
              ...
              photos: (() => {
                ...
                return photosArray.map((photo: any, index: number) => ({
                  id: `${
                    submission.id || submission.transaction_id
                  }_photo_${index}`,
        ...
        if (error.name === "AbortError") {
          console.error("Request timeout - materials fetch took too long");
        } else {
    ...
    // }));
  }, [user]); // Removed unnecessary dependencies to prevent re-fetching
  ...
  const filteredMaterials = backendMaterials.filter(
    (material) =>
          ...
          <div className="mt-3 flex items-center gap-4">
            <div className="inline-flex items-center px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-full">
              ...
              <span className="text-sm font-medium text-emerald-700 dark:text-emerald-200">
                {backendMaterials.length} submissions
              </span>
            </div>
            <div className="inline-flex items-center px-3 py-1.5 bg-gray-50 dark:bg-gray-700/40 rounded-full">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-300">
                Est. value LKR{" "}
                {backendMaterials
                  .reduce((s, m) => s + (m.pricing?.expectedPrice || 0), 0)
                  .toLocaleString()}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              onChange={(e) =>
                setGridDensity(
                  e.target.value as "comfortable" | "compact" | "dense"
                )
              }
...
function MaterialCard({
  ...
  const mainPhoto =
    material.photos.find((photo: any) => photo.isMain) || material.photos[0];
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-20">
        <div
          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm ${getStatusColor(
            material.status
          )}`}
        >
      ...
      {/* Image */}
      <div
        className={`relative ${densityClasses} bg-gray-200 dark:bg-gray-700`}
      >
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {material.condition
                ? material.condition.charAt(0).toUpperCase() +
                  material.condition.slice(1)
                : "Unknown"}
            </div>
            <button
              onClick={onView}
...
export function QualityBadge({ grade }: QualityBadgeProps) {
  const gradeConfig = {
    [QualityGrade.EXCELLENT]: {
      color:
        "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400 ring-1 ring-emerald-600/20",
      icon: "⭐",
      label: "Excellent",
    },
    [QualityGrade.GOOD]: {
      color:
        "bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400 ring-1 ring-blue-600/20",
      icon: "✅",
      label: "Good",
    },
    [QualityGrade.FAIR]: {
      color:
        "bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400 ring-1 ring-amber-600/20",
      icon: "⚠️",
      label: "Fair",
    },
    [QualityGrade.POOR]: {
      color:
        "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400 ring-1 ring-red-600/20",
      icon: "❌",
      label: "Poor",
    },
  };
  ...
  const config = gradeConfig[grade] || {
    color:
      "bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400 ring-1 ring-gray-600/20",
    icon: "❓",
    label: grade || "Unknown",
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <select className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-emerald-500 focus:border-emerald-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
