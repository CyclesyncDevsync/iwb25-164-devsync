...
const MapContainer = dynamic(
  () => import("react-leaflet").then((mod) => mod.MapContainer),
  {
    ssr: false,
    loading: () => (
      <div className="h-[400px] bg-gray-100 dark:bg-gray-700 animate-pulse rounded-lg" />
    ),
  }
);
...
export function MaterialVerification() {
  ...
  const [selectedSubmissionForAgent, setSelectedSubmissionForAgent] =
    useState<MaterialSubmission | null>(null);
  ...
  const processSubmissionsData = (
    submissionsData: any[],
    agentsList: Agent[] = agents
  ) => {
    const apiSubmissions: MaterialSubmission[] = submissionsData.map(
      (item: any) => ({
        id: item.id,
        ...
        agent_assigned:
          item.agent_assigned === true || item.agent_id ? true : false,
        ...
        assigned_agent: (() => {
          ...
          if (agentId) {
            ...
            console.log(
              `Agent not found for ${item.title}, agent_id: ${agentId}`
            );
            console.log(
              "Available agents:",
              agentsList.map((a) => ({
          ...
          if (typeof item.photos === "string") {
            try {
              ...
              if (Array.isArray(parsed)) {
                return parsed.map((p: any) => p.data || p);
              }
            } catch (e) {
    ...
    const stats = {
      ...
      agentVisits: apiSubmissions.filter(
        (s) => s.delivery_method === "agent_visit"
      ).length,
      dropOffs: apiSubmissions.filter((s) => s.delivery_method === "drop_off")
        .length,
      pendingVerifications: apiSubmissions.filter(
        (s) => s.submission_status === "pending_verification"
      ).length,
    };
    setStatistics(stats);
  };
  ...
  const fetchMaterialSubmissionsWithAgents = async (agentsList: Agent[]) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "Assigned agent for first item:",
          data.data?.[0]?.assigned_agent
        ); // Debug agent field

        // Log all items that have assigned status
        const assignedItems = data.data.filter(
          (item: any) => item.submission_status === "assigned"
        );
  ...
  const agentVisitSubmissions = filteredSubmissions.filter(
    (sub) => sub.delivery_method === "agent_visit"
  );
  const dropOffSubmissions = filteredSubmissions.filter(
    (sub) => sub.delivery_method === "drop_off"
  );
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium ${
          statusStyles[status] || statusStyles.pending_verification
        }`}
      >
    ...
    try {
      ...
      if (response.ok) {
        ...
        const agentUsers = allUsers.filter(
          (user: any) => user.role?.toLowerCase() === "agent"
        );
        ...
        console.log(
          "Processed agents with asgardeo_ids:",
  ...
  const fetchWarehouseStats = () => {
    try {
      ...
      const dropOffs = submissions.filter(
        (s) => s.delivery_method === "drop_off"
      );
      console.log(
        "Drop-off submissions for warehouse calculation:",
        dropOffs.length
      );
      ...
      dropOffs.forEach((submission) => {
        ...
        if (submission.selected_warehouse_name) {
          ...
          if (!warehouseMap.has(key)) {
            ...
            const matchingCenter = WAREHOUSE_CENTERS.find(
              (w) => w.name === key
            );
            warehouseMap.set(key, {
              ...
              address:
                submission.selected_warehouse_address || "Unknown Address",
      ...
      calculatedWarehouses.forEach((calcWarehouse) => {
        const index = allWarehouses.findIndex(
          (w) => w.name === calcWarehouse.name
        );
        if (index !== -1) {
  ...
  const assignAgent = async (agentId: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/test-assign", {
        ...
        body: JSON.stringify({
          ...
          location: {
            ...
            address:
              selectedSubmissionForAgent.location_address || "Unknown location",
          },
        }),
      });
      ...
      toast.error(
        "Failed to assign agent. Please check your connection and try again.",
        {
          duration: 4000,
          position: "top-right",
        }
  ...
  const analyzePhoto = async (photo: string, index: number) => {
    ...
    try {
      ...
      const analysisRequest = {
        ...
        fieldAgentId:
          selectedSubmissionForDetails.assigned_agent?.id?.toString() ||
          "unassigned",
      ...
      setPhotoAnalysisResults((prev) => ({
        ...prev,
        [index]: { error: "Analysis error", message: errorMessage },
      }));
    } finally {
        ...
        <div className="h-[400px] rounded-lg overflow-hidden">
          ...
          >
            ...
            {agentVisitSubmissions.map((submission) => {
              ...
              if (
                submission.location_latitude &&
                submission.location_longitude
              ) {
                return (
                  <CircleMarker
                    key={submission.id}
                      ...
                      <div className="p-2">
                        ...
                        {submission.assigned_agent && (
                          <p className="text-sm">
                            Agent:{" "}
                            <span className="font-medium">
                              {submission.assigned_agent.name}
                            </span>
                          </p>
                        )}
                      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Material:{" "}
                <span className="font-medium">
                  {selectedSubmissionForAgent?.title}
                </span>
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Delivery:{" "}
                <span className="font-medium">
                  {selectedSubmissionForAgent?.delivery_method === "agent_visit"
                    ? "Agent Visit"
              ...
              {selectedSubmissionForAgent?.assigned_agent && (
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  ...
                  <span className="font-medium">
                    {selectedSubmissionForAgent.assigned_agent.name}
                  </span>
                </p>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <p className="text-sm">
                    ...
                    <span className="font-medium">
                      {selectedSubmissionForDetails.transaction_id}
                    </span>
                  </p>
                  <p className="text-sm">
                  ...
                  <p className="text-sm">
                    ...
                    <span className="font-medium">
                      {selectedSubmissionForDetails.category}
                    </span>
                  </p>
                  ...
                  <p className="text-sm">
                    ...
                    <span className="font-medium">
                      {selectedSubmissionForDetails.quantity}{" "}
                      {selectedSubmissionForDetails.unit}
                    </span>
                  </p>
                  <p className="text-sm">
                    <span className="text-gray-500">Status:</span>{" "}
                    {getStatusBadge(
                      selectedSubmissionForDetails.submission_status
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <p className="text-sm">
                    ...
                    <span className="font-medium">
                      {selectedSubmissionForDetails.supplier_name || "Unknown"}
                    </span>
                  </p>
                  <p className="text-sm">
                    <span className="text-gray-500">Email:</span>{" "}
                    <span className="font-medium">
                      {selectedSubmissionForDetails.supplier_email || "N/A"}
                    </span>
                  </p>
                  <p className="text-sm">
                    <span className="text-gray-500">Supplier ID:</span>{" "}
                    <span className="font-medium">
                      {selectedSubmissionForDetails.supplier_id}
                    </span>
                  </p>
                </div>
              </div>
              ...
              {/* Material Photos */}
              {selectedSubmissionForDetails.photos &&
                Array.isArray(selectedSubmissionForDetails.photos) &&
                selectedSubmissionForDetails.photos.length > 0 && (
                  <div className="space-y-4 col-span-1 md:col-span-2">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                      Photos
                    </h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {selectedSubmissionForDetails.photos.map(
                        (photo, index) => {
                                    ...
                                    <>
                                      ...
                                      <div className="space-y-2">
                                        <div className="flex items-start gap-2">
                                          ...
                                          {photoAnalysisResults[index]
                                            .qualityFactors?.sorting
                                            ?.materialTypeMatches === true ||
                                          (photoAnalysisResults[index]
                                            .qualityFactors?.sorting
                                            ?.materialTypeMatches ===
                                            undefined &&
                                        ...
                                        {photoAnalysisResults[index]
                                          ...
                                          <div className="mt-2 pl-7 space-y-1">
                                            ...
                                            {photoAnalysisResults[index]
                                              .qualityFactors?.contamination
                                              ?.contaminants?.length > 0 && (
                                              <p className="text-xs text-amber-600 dark:text-amber-400">
                                                ⚠️ Contaminants detected:{" "}
                                                {photoAnalysisResults[
                                                  index
                                                ].qualityFactors.contamination.contaminants.join(
                                                  ", "
                                                )}
                                              </p>
                                            )}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <p className="text-sm">
                    ...
                    <span className="font-medium">
                      {selectedSubmissionForDetails.delivery_method ===
                      "agent_visit"
                        ? "Agent Visit"
                        : "Drop-off"}
                    </span>
                  ...
                  "agent_visit" ? (
                    <>
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.location_address ||
                            "N/A"}
                        </span>
                      </p>
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.location_city || "N/A"}
                        </span>
                      </p>
                      <p className="text-sm">
                        <span className="text-gray-500">District:</span>{" "}
                        <span className="font-medium">
                          {selectedSubmissionForDetails.location_district ||
                            "N/A"}
                        </span>
                      </p>
                    </>
                    ...
                    <>
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.selected_warehouse_name ||
                            "N/A"}
                        </span>
                      </p>
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.selected_warehouse_address ||
                            "N/A"}
                        </span>
                      </p>
                    </>
                  ...
                  <div className="space-y-2">
                    {selectedSubmissionForDetails.dimension_length && (
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.dimension_length} cm
                        </span>
                      </p>
                    )}
                    {selectedSubmissionForDetails.dimension_width && (
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.dimension_width} cm
                        </span>
                      </p>
                    )}
                    {selectedSubmissionForDetails.dimension_height && (
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.dimension_height} cm
                        </span>
                      </p>
                    )}
                    {selectedSubmissionForDetails.dimension_weight && (
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.dimension_weight} kg
                        </span>
                      </p>
                    )}
                  </div>
                  ...
                  {selectedSubmissionForDetails.assigned_agent ? (
                    <>
                      <p className="text-sm">
                        ...
                        <span className="font-medium">
                          {selectedSubmissionForDetails.assigned_agent.name}
                        </span>
                      </p>
                      <p className="text-sm">
                      ...
                      {selectedSubmissionForDetails.delivery_method ===
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 italic">
                          Agent will coordinate drop-off logistics with the
                          supplier
                        </p>
                      )}
                    </>
            ...
            <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
              <p className="text-sm text-gray-500">
                Submitted on:{" "}
                {format(
                  new Date(selectedSubmissionForDetails.created_at),
                  "PPP"
                )}{" "}
                at{" "}
                {format(new Date(selectedSubmissionForDetails.created_at), "p")}
              </p>
            </div>
