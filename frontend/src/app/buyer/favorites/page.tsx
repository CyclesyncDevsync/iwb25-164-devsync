...
interface FavoriteMaterial extends Material {
  favoritedAt: Date;
  notes?: string;
}

const BuyerFavoritesPage = () => {
  ...
  // Mock favorite materials data with proper Supplier objects
  const [favoriteMaterials, setFavoriteMaterials] = useState<FavoriteMaterial[]>([
    {
      id: '1',
      ...
      notes: 'High quality material, good for our production line'
    },
    {
      id: '2',
      ...
      isSaved: true,
      verificationStatus: 'verified',
      favoritedAt: new Date('2024-01-12'),
      notes: 'Consistent supplier, reliable quality'
    },
    {
      id: '3',
    ...
    },
    {
      ...
      description: 'Sorted glass bottles, various colors available.',
      category: 'glass',
      price: 12,
      unit: 'kg',
      quality: 8,
      ...
      isSaved: true,
      verificationStatus: 'verified',
      favoritedAt: new Date('2024-01-08'),
      notes: 'Good variety, competitive pricing'
    }
  ]);
  ...
  const filteredAndSortedMaterials = useMemo(() => {
    const filtered = favoriteMaterials.filter(material => {
    ...
    return filtered;
  }, [favoriteMaterials, searchQuery, selectedCategory, sortBy]);
  ...
  const handleRemoveFavorite = async (materialId: string) => {
    ...
    setFavoriteMaterials(prev => prev.filter(material => material.id !== materialId));
    setIsLoading(false);
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-green-100 dark:bg-green-900/20 rounded-lg">
                <HeartSolidIcon className="h-6 w-6 text-green-600 dark:text-green-400" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">My Favorites</h1>
                <p className="text-gray-600 dark:text-gray-400">
                  {favoriteMaterials.length} saved material{favoriteMaterials.length !== 1 ? 's' : ''}
                </p>
              </div>
            </div>
        ...
        {filteredAndSortedMaterials.length === 0 ? (
          ...
          >
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Start exploring materials and save your favorites for quick access.
            </p>
            <div className="mt-6">
          ...
          <div className={`grid gap-6 ${
            ...
            {filteredAndSortedMaterials.map((material, index) => (
              ...
              >
                ...
                {/* Favorite date and notes */}
                <div className="mt-2 px-4 pb-4">
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    Added {new Date(material.favoritedAt).toLocaleDateString()}
                  </div>
                  {material.notes && (
