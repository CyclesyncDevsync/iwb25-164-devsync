...
const BACKEND_ADMIN_URL =
  process.env.NEXT_PUBLIC_ADMIN_API_URL || "http://localhost:8093";

export async function GET(request: NextRequest) {
  try {
    // Check cache first
    const cacheKey = "admin:agents:list";
    const cachedAgents = await cache.get(cacheKey);

    if (cachedAgents) {
      console.log("✅ Cache HIT for agents list - INSTANT LOAD");
    ...
    console.log("⚠️ Cache MISS for agents list - Fetching from backend...");
    ...
    const backendResponse = await fetch(
      `${BACKEND_ADMIN_URL}/api/admin/users?role=agent&limitParam=${limit}&offsetParam=${offset}`,
      {
        method: "GET",
        headers: {
    ...
    // Cache for 5 minutes (300 seconds) - Agents don't change frequently
