          ...
          if (data.success && data.assignments) {
            ...
            const transformedAssignments = data.assignments.map(
              (assignment: BackendAssignment) => ({
                ...
                title:
                  assignment.materialDetails?.title || "Material Verification",
                location: {
                  address:
                ...
                estimatedTime: assignment.agent?.costBreakdown
                  ?.estimatedDuration
                  ? Math.ceil(
                      assignment.agent.costBreakdown.estimatedDuration * 60
                    )
                  : 45,
                supplier: {
                  name: assignment.supplierName || "Unknown Supplier",
                  contact:
                    assignment.agent?.agentPhone || "Contact not available",
                },
                materials: assignment.materialDetails
                  ? [
                      {
                        type: assignment.materialDetails.type || "Material",
                        quantity: assignment.materialDetails.quantity
                          ? parseFloat(assignment.materialDetails.quantity)
                          : 1,
                        unit: "kg",
                      },
                    ]
                  : [],
                deadline: assignment.createdAt
            ...
            const todayAssignments = transformedAssignments.filter(
            ...
            const weeklyAssignments = transformedAssignments.filter(
              (assignment: Assignment) => {
                const assignmentDate = new Date(assignment.deadline);
            ...
            setStats({
              ...
              todayPending: todayAssignments.filter(
